function [spls, removed_spls, spls_density, A, corresp] = edge_collapse_update(pts, spls, spls_density, corresp, neigh, spls_adj, options)
    ...
    removed_spls = zeros(0, 3); %[ori pts 1; ori pts 2; updated pts; ...; so forth]
    A = spls_adj;
    A(A > 0) = 1;
    ...
    while true
        ...
        % update the location
        removed_spls = [removed_spls; spls(edge, :)];
        spls(edge(2), :) = mean(spls(edge, :));
        spls(edge(1), :) = NaN;
        spls_density(edge(2), :) = mean(spls_density(edge, :));
        spls_density(edge(1), :) = 0;
        removed_spls = [removed_spls; spls(edge(2), :)];
        % update the A matrix
        for k = 1:size(A, 1)
